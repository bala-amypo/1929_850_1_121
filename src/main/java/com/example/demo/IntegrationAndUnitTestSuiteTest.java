...
public class IntegrationAndUnitTestSuiteTest {
    ...
    public void test03_createStudent_invalidYear_fail() {
        Student s = Student.builder().rollNumber("R101").name("Bob").department("CS").year(6).build();
        try {
    ...
    public void test16_student_unique_roll_check() {
        ...
        try {
            studentService.addStudent(Student.builder().rollNumber("R200").name("X").department("CS").year(3).build());
            Assert.fail();
        } catch (ApiException ex) {
            Assert.assertTrue(ex.getMessage().toLowerCase().contains("already"));
        }
    }
    ...
    public void test38_session_requires_students_fail() {
        try {
            ...
            Assert.assertTrue(ex.getMessage().toLowerCase().contains("students are required"));
        }
    }
    ...
    public void test57_plan_arrangement_json_valid() throws Exception {
        ...
        new ObjectMapper().readTree(p.getArrangementJson());
    }
